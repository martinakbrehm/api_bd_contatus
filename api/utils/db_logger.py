            ...
            try:
                ...
                cur.execute(
                    ...
                    INSERT INTO api_log_consultas (
                        request_id, key_id, nome_usuario, role, ip,
...
def extrair_campos_auth(auth: dict) -> tuple[Optional[str], Optional[str]]:
    """
    Extrai (key_id, nome_usuario) do contexto g.auth_user.

    - API Key: key_id = subject (prefixo lspf_...), nome_usuario = key_nome
    - JWT:     key_id = None,                       nome_usuario = subject
    """
    if auth.get("auth_method") == "api_key":
        return auth.get("subject"), auth.get("key_nome")
    return None, auth.get("subject")
